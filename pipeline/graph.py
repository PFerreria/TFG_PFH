...
pipeline/graph.py
-----------------
LangGraph State Machine — full deterministic workflow controlling agent execution.

Pipeline flow:
  START
    │
    ▼
  [tts_node]          → transcribe audio to text
    │
    ▼
  [#nlp_node]          → classify incident + extract location (parallel tools, single node)
    │
    ▼
  [validate_node]     → check location was resolved
    │
    ├--- location OK ----------------------------------┐
    │                                                   ▼
    └--- location FAIL → [geo_retry_node]          [fan_out]
              │               │                        │
              └---------------┘              ┌---------┴---------┐
                                             ▼         ▼         ▼
                                      [procedure] [dispatch]   [geo]
                                          (parallel async nodes)
                                             │         │         │
                                             └----┬----┘---------┘
                                                  ▼
                                            [merge_node]
                                                  ▼
                                           [output_node]  → build final report
                                                  │
                                                 END

  AnalysisAgent runs on TWO separate paths — never inside this graph:
    Path A: APScheduler (every 60 min) → generate_dashboard_data() → cached in Redis/DB
    Path B: post-dispatch hook on IMERSPipeline → annotate_incident_async() → fire-and-forget thread
    See: analysis_runner.py for both.

Fan-out / merge pattern:
  LangGraph does not have native parallel execution in Python sync mode.
  We implement parallel execution using asyncio.gather() inside fan_out_node,
  which runs procedure_node, dispatch_node, and geo_node concurrently.
  The merge_node then assembles all three results from state.

Conditional edges:
  - validate_node  → "fan_out" | "geo_retry"
  - geo_retry_node → "fan_out" | "abort"   (max 1 retry)
...
class IMERSState(TypedDict, total=False):
    ...
    the fields they modify — LangGraph merges partial updates automatically.

    Naming convention:
      - Raw inputs:      audio_path, transcript
      - Classification:  incident_type, severity, victims, classification_confidence
      - Location:        location_raw, location_address, location_lat, location_lon,
                         location_confidence, location_validated, geo_retry_count
      - Agent outputs:   procedure_result, dispatch_result, geo_result, analysis_result
      - Control flow:    fan_out_done, merge_done, error, abort_reason
      - Metadata:        incident_id, started_at, completed_at, node_timings
    """
...
def _initial_state(
    ...
    """Create a fresh state for a new pipeline invocation.

    Pass *incident_id* to reuse an existing ID (e.g. when a CallSession runs
    several pipeline passes for the same call).  Omit it to let the pipeline
    generate a unique ID automatically.

    Pass *is_preliminary=True* for early-trigger runs so fan_out_node skips
    LLM agents and uses direct tool fallbacks instead.
    """
    return IMERSState(
...
def _timing(state: IMERSState, node: str, elapsed: float) -> dict:
    """Return a partial state update that merges the node's elapsed time into node_timings."""
...
def tts_node(state: IMERSState, agents: dict) -> IMERSState:
    ...
    If transcript is already in state (injected directly), skip Whisper entirely.
    """
...
def nlp_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Run classify_incident + extract_location against the transcript.
    These two tool calls are independent so we run them concurrently with asyncio.
    Produces: incident_type, severity, victims, location_* fields.
    """
    t0         = time.perf_counter()
...
def validate_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Validate that a usable location was resolved.
    Sets location_validated = True/False.
    The conditional edge after this node reads location_validated.
    """
    t0 = time.perf_counter()
...
def geo_retry_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Retry geocoding using a broader query.
    Called when validate_node finds no usable coordinates.
    Tries: (1) combining all candidates into one query, (2) city-level fallback.
    """
    t0 = time.perf_counter()
    from tools.extract_location import extract_location
...
def fan_out_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Run Procedure, Dispatch, and Geo agents concurrently using ThreadPoolExecutor.

    Environment variables:
      IMERS_FAST_MODE=1              — bypass LLM, use rule-based fallbacks (< 1 s total).
      IMERS_AGENT_TIMEOUT_SECS=120   — per-agent hard timeout; falls back to _fallback() on expiry.

    AnalysisAgent is intentionally absent — it runs on its own schedule and as a
    post-dispatch hook. See agents/analysis_agent.py and analysis_runner.py.
    """
    t0 = time.perf_counter()
    ...
    def _geo_fallback(reason: str) -> dict:
        """Return a geo result using direct tool calls instead of the LLM agent."""
    ...
    if fast_mode:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
            ...
            def _collect(future, name: str, fallback_fn):
                """Wait for a future with timeout; call fallback_fn on expiry or error."""
                try:
...
def merge_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Merge all three agent results (procedure, dispatch, geo) into a coherent,
    validated state.  Reconciles any conflicts (e.g. geo agent found a better
    address than nlp_node).  Adds warnings for missing or error results.
    """
    t0       = time.perf_counter()
    warnings = []
...
def output_node(state: IMERSState, agents: dict) -> IMERSState:
    """
    Assemble the final incident report from all state fields.
    This is what the operator dashboard and the LangGraph caller receive.
    """
    t0       = time.perf_counter()
    ...
    report = {
        ...
        "transcript_preview": (state.get("transcript") or "")[:120],
    }
...
def abort_node(state: IMERSState, agents: dict) -> IMERSState:
    ...
    Builds a minimal report with the abort reason so callers are not left hanging.
    """
...
def route_check_abort(state: IMERSState) -> str:
    """Shared guard: if any node set abort_reason, go straight to abort_node."""
...
def build_graph(agents: dict, checkpointer=None) -> Any:
    ...
    Usage:
        ...
        agents = {
            "tts":       TTSAgent(),
            "procedure": ProcedureAgent(model),
            "dispatch":  DispatchAgent(model),
            "geo":       GeoAgent(model),
            # "analysis" is intentionally absent — see IMERSPipeline for its lifecycle
        }
        app = build_graph(agents, checkpointer=MemorySaver())
    """
...
class IMERSPipeline:
    ...
    Manages agent initialisation, state creation, and invocation.

    The pipeline owns exactly four agents: TTS, Procedure, Dispatch, Geo.
    The AnalysisAgent is deliberately excluded — it is managed by
    analysis_runner.py, which handles its two independent lifecycles:
      1. Scheduled dashboard refresh  (APScheduler, every 30-60 min)
      2. Post-dispatch annotation      (fire-and-forget thread per incident)

    To wire post-dispatch annotation, register a callback before running calls:

        pipeline = IMERSPipeline(hf_token="hf_EXAMPLE")
        pipeline.add_post_dispatch_hook(
            lambda report: analysis_agent.annotate_incident_async(
                incident_id=report["incident_id"],
                incident_type=report["incident_type"],
                lat=report["location"]["latitude"],
                lon=report["location"]["longitude"],
                on_complete=lambda a: db.save_annotation(a),
            )
        )

    Usage:
        pipeline = IMERSPipeline(hf_token="hf_EXAMPLE")
        report   = pipeline.run_transcript("Ha habido un accidente en...")
        state    = pipeline.last_state   # full state for debugging
    """
    ...
    def add_post_dispatch_hook(self, callback) -> None:
        """
        Register a callback to be called with the final_report after every
        successful pipeline run. Callbacks run in a background daemon thread
        so they never block the caller.

        Args:
            callback: Callable[[dict], None] — receives the final_report dict.

        Example (wiring analysis annotation):
            pipeline.add_post_dispatch_hook(
                lambda report: analysis_agent.annotate_incident_async(
                    incident_id   = report["incident_id"],
                    incident_type = report["incident_type"],
        ...
        Example (WebSocket push to dashboard):
            pipeline.add_post_dispatch_hook(
                lambda report: asyncio.run(ws_manager.broadcast(
                    {"type": "new_incident", "data": report}
                ))
            )
        """
    ...
    def run_transcript(
        ...
        """Process a pre-transcribed call (skips Whisper).

        Pass *is_preliminary=True* for early-trigger runs so fan_out_node
        skips LLM agents and uses direct tool fallbacks instead.
        """
    ...
    def _invoke(self, state: IMERSState) -> dict:
        """Invoke the compiled graph, fire post-dispatch hooks on success, and return the final report."""
        t0 = time.perf_counter()
        try:
    ...
    def _run_hook(hook, report: dict) -> None:
        """Execute a single post-dispatch hook, catching and logging any error."""
        try:
            hook(report)
        except Exception as e:
