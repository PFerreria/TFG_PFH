...
def get_route(
    ...
    """Calculates the fastest emergency driving route between two locations.
    Returns turn-by-turn directions, total distance, and estimated travel time.
    Accepts either addresses (geocoded automatically) or pre-resolved coordinates.

    Args:
        destination_address: Address of the incident, e.g. 'Calle Sierpes 14, Sevilla'.
                             Ignored if destination_lat and destination_lon are non-zero.
        origin_address: Address of the unit's starting location, e.g. 'Base Sur, Sevilla'.
                        Ignored if origin_lat and origin_lon are non-zero, or if unit_type is used to find nearest.
        unit_type: Optional. The type of unit ('ambulance_sva', 'police', 'fire', 'rescue').
                   If provided, overriding origin_address to find the nearest base dynamically.
        is_return_trip: Optional. If True and unit is an ambulance, reduces speed to protect the victim.
        origin_lat: Latitude of origin (optional — skips geocoding if provided).
        origin_lon: Longitude of origin (optional — skips geocoding if provided).
        destination_lat: Latitude of destination (optional — skips geocoding if provided).
        destination_lon: Longitude of destination (optional — skips geocoding if provided).

    Returns:
        A JSON string with keys:
          - backend (str): routing engine used — 'openrouteservice', 'osmnx', or 'stub_straight_line'
          - distance_km (float): total route distance in kilometres
          - duration_minutes (float): estimated driving time (emergency speed)
          - instructions (list): turn-by-turn steps with distance and duration
          - polyline (str | null): encoded polyline geometry for map rendering
          - origin_coords (list): [lat, lon] used for routing
          - destination_coords (list): [lat, lon] used for routing
          - error (str | null): error message if routing partially failed
        Example:
          '{"backend": "openrouteservice", "distance_km": 3.2, "duration_minutes": 5.4,
            "instructions": [{"step": 1, "instruction": "Head north on Calle Sierpes", ...}],
            "polyline": "...", "origin_coords": [37.38, -5.99],
            "destination_coords": [37.39, -5.98], "error": null}'
    """
    error_msg = None
