...
def _available_units(unit_type: str,
    ...
    """Return available units of the given type, sorted by computed ETA.

    Units that are statically marked busy/en_route in the registry OR
    currently tracked as dispatched are excluded.  ETA is computed from
    base→incident distance using the unit's emergency speed.  When incident
    coordinates are not provided, eta_minutes is set to None.
    """
...
def recommend_units(
    ...
    """Recommends and allocates emergency units to dispatch based on incident type,
    severity, and victim count. Checks real-time unit availability and returns
    the optimal set of units with estimated arrival times.

    Args:
        incident_type: Type of emergency — e.g. 'traffic_accident', 'cardiac_arrest',
                       'fire', 'stroke', 'assault'. Must match classify_incident output.
        severity: Severity level — one of 'critical', 'high', 'medium', 'low'.
        location: Incident address or area (used for ETA calculation context).
        victims: Estimated number of victims. Triggers additional units if > 3.
        incident_lat: Latitude of the incident location (decimal degrees). Used for
                      accurate ETA calculation. Defaults to 0.0 (uses jitter fallback).
        incident_lon: Longitude of the incident location (decimal degrees). Used for
                      accurate ETA calculation. Defaults to 0.0 (uses jitter fallback).

    Returns:
        A JSON string with keys:
          - dispatched (list): units to send, each with id, type, eta_minutes, base_location
          - unavailable (list): unit types that were requested but had no available units
          - warnings (list): advisory messages e.g. insufficient units
          - total_units (int): total number of units being dispatched
          - estimated_first_arrival (int): ETA of the fastest unit in minutes
        Example:
          '{"dispatched": [{"id": "AMB-SVA-01", "type": "ambulance_sva", "eta_minutes": 7,
             "base_location": "Hospital Virgen del Rocío"}],
            "unavailable": [], "warnings": [], "total_units": 1, "estimated_first_arrival": 7}'
    """
