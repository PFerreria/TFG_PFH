...
def _extract_victim_count(transcript: str) -> int:
    """
    Extract victim count from transcript.
    Strategy:
      1. Find all explicit numeric/word-count mentions tied to victim nouns (Pattern 1).
      2. Find all singular "un/una/otro/otra" + IMMEDIATE victim adjective mentions (Pattern 2),
         but only when NOT already captured by Pattern 1.
    Returns total victim count.
    """
    total_victims = 0
    p1_spans = [] 
...
def _classify_by_rules(transcript: str) -> Optional[Classification]:
    """
    Score-based classification using weighted voting.

    Each matching rule votes for an incident type with its weight and priority.
    The type with the highest total score wins. Within a type, the highest-priority
    (lowest numeric index) matched rule sets the base priority.
    This avoids the old bug where a single low-specificity rule (e.g., 'inconsciente'
    for cardiac_arrest) would always override a richer cluster of traffic keywords.
    """
...
def classify_incident(transcript: str) -> str:
    ...
    Uses keyword pattern matching for speed. Returns incident type, typology, priority, estimated victim
    count, and confidence level. This output drives both the Dispatch and Procedure agents.
