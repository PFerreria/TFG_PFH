"""
test_recommend_units.py
-----------------------
Comprehensive unit tests for tools/recommend_units.py.

Covers:
  - _is_dispatched / _mark_dispatched / release_units  (dispatch tracking)
  - _available_units                                    (registry filtering)
  - _get_rules                                          (rule lookup + fallback)
  - recommend_units                                     (public tool, JSON output)

Run with:
    pytest test/test_recommend_units.py -v
"""
...
def _clear_dispatch_state():
    """Reset the global dispatch tracking dict between tests."""
    with _dispatch_lock:
        _dispatched.clear()
...
def reset_dispatch():
    """Ensure a clean dispatch state before each test."""
...
class TestDispatchTracking:
    ...
    def test_ttl_zero_eta_treated_as_immediately_expired(self):
        """
        ETA of 0 → TTL is 0 → unit should not be locked for any measurable time.
        We mark with ETA=0 and then check: the state may or may not consider it
        dispatched depending on how quickly monotonic() advances — we just ensure
        no crash.
        """
...
class TestAvailableUnits:
    ...
    def test_busy_units_excluded(self):
        """Units with status != 'available' in the registry must not appear."""
        ambulances = _available_units("ambulance_svb")
        for u in ambulances:
            assert u.status == "available"

    def test_dispatched_units_excluded(self):
        """Units marked dispatched must not appear in available list."""
    ...
    def test_eta_jitter_positive(self):
        """ETA with jitter must always be >= 1."""
        for unit_type in ("ambulance_svb", "police", "fire"):
            for unit in _available_units(unit_type):
...
class TestGetRules:
    ...
    def test_fallback_to_highest_severity(self):
        """If 'cardiac_arrest/low' not defined, should fall back to a defined severity."""
...
class TestRecommendUnits:
    ...
    def test_estimated_first_arrival_is_min_eta(self):
        ...
        if result["dispatched"]:
            etas = [u["eta_minutes"] for u in result["dispatched"]]
            assert result["estimated_first_arrival"] == min(etas)

    def test_estimated_first_arrival_none_when_no_dispatch(self):
        """If all units of the needed type are busy, first_arrival may be None."""
    ...
    def test_dispatch_marks_units_as_busy(self):
        """After a dispatch call, units chosen should be marked as dispatched."""
    ...
    def test_many_victims_increase_units(self):
        """4+ victims should trigger extra unit scaling."""
    ...
    def test_critical_late_arrival_warning(self):
        """
        If all fast units are taken, the first-arrival ETA might exceed 10 min
        for a critical incident → expect a warning.
        We trigger this by exhausting all fast units.
        """
        for u in _UNIT_REGISTRY:
            if u.eta_minutes <= 5:
                _mark_dispatched(u.id, eta_minutes=60)
    ...
    def test_all_incident_types_in_mapping(self):
        """recommend_units should handle every incident type without crashing."""
        incident_types = [
