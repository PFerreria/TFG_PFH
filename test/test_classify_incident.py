...
--------------------------
Comprehensive unit tests for tools/classify_incident.py.

Covers:
  - _extract_victim_count      (explicit counts, word numbers, fallback heuristic)
  - _bump_severity             (up / down / boundary clamping)
  - _classify_by_rules         (happy paths, multi-match voting, severity modifiers)
  - classify_incident          (public tool: JSON output, edge cases, empty input)

Run with:
    pytest test/test_classify_incident.py -v
"""
...
class TestExtractVictimCount:
    ...
    def test_fallback_he_not_breathing(self):
        """'He is not breathing' implies at least 1 victim via heuristic fallback."""
...
class TestBumpSeverity:
    ...
    def test_bump_zero(self):
        """A bump of 0 should leave the level unchanged."""
        for level in PRIORITY_LEVELS:
            assert _bump_severity(level, 0) == level

    def test_all_levels_covered(self):
        """Every priority level should be bumpable without KeyError."""
        for level in PRIORITY_LEVELS:
...
class TestClassifyByRules:
    ...
    def test_severity_upgrade_on_unconscious(self):
        """'inconsciente' should push severity up."""
        result = _classify_by_rules(
    ...
    def test_weighted_voting_traffic_wins_over_generic_inconsciente(self):
        """
        Multiple traffic keywords + 'inconsciente' should classify as traffic_accident,
        not cardiac_arrest, because traffic has higher total vote weight.
        """
        result = _classify_by_rules(
    ...
    def test_incident_mapping_populated(self):
        """Every incident_type returned by rules must exist in INCIDENT_MAPPING."""
        transcripts = [
            "Hay un incendio.",
            "Mi padre no respira.",
