...
- Forces UTF-8 stdout/stderr on Windows (avoids charmap errors with Spanish text).
- Provides shared fixtures reused across test files.

NOTE: sys.path is configured via pytest.ini (pythonpath = .) rather than here,
to avoid shadowing stdlib modules when the project root directory is named 'code'.
"""
...
def project_root() -> Path:
    """Return the absolute path to the project root (code/)."""
    return PROJECT_ROOT
...
def sample_transcript_es() -> str:
    """A realistic Spanish emergency call transcript used across multiple tests."""
    return (
...
def sample_transcript_en() -> str:
    """A realistic English emergency call transcript."""
    return (
...
def stub_classification() -> dict:
    """A pre-built classification result dict."""
    return {
...
def stub_location() -> dict:
    """A pre-built location result dict."""
    return {
        "found": True,
...
def stub_dispatch_result() -> dict:
    """A pre-built dispatch result dict."""
    return {
        "dispatched": [
