...
test_tts_agent.py
-----------------
Comprehensive unit tests for agents/tts_agent.py.

Covers:
  - TranscriptionResult        (dataclass helpers: to_dict, to_json)
  - TTSAgent._resolve_audio_path   (absolute, relative with .., simple filename)
  - TTSAgent._validate_audio_file  (existence, extension, large-file warning)
  - TTSAgent.run               (error path, backend fallback chain, success)
  - TTSAgent.run_to_dict       (delegates to run)
  - TTSAgent.run_to_json       (returns valid JSON string)

Run with:
    pytest test/test_tts_agent.py -v
"""
...
def _make_wav(path: Path, duration: float = 1.0, frequency: float = 440.0) -> Path:
    """Write a minimal valid WAV file (sine wave) to *path*. Returns *path*."""
    sr = 16000
...
class TestValidateAudioFile:
    ...
    def test_large_file_logs_warning_not_error(self, tmp_path):
        """Large files should trigger a logger.warning but return None (not an error)."""
        p = tmp_path / "big.wav"
        _make_wav(p)
...
class TestTTSAgentRun:
    ...
    def test_all_backends_fail_returns_error(self, tmp_path):
        """When every Whisper backend is unavailable, run() returns an error result."""
    ...
    def test_groq_exception_falls_back_to_faster(self, tmp_path):
        """If Groq fails, faster-whisper should be tried next."""
        p = _make_wav(tmp_path / "call.wav")
    ...
    def test_simple_filename_resolved_to_default_folder(self, tmp_path, monkeypatch):
        """run("call.wav") should resolve to DEFAULT_AUDIO_FOLDER/call.wav."""
