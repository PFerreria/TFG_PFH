...
test_get_route.py
-----------------
Comprehensive unit tests for tools/get_route.py.

Covers:
  - _in_sevilla_bounds          (boundary logic)
  - _haversine_km               (distance formula)
  - _encode_polyline            (encoding correctness)
  - _route_stub                 (straight-line fallback)
  - _geocode_address            (cache hit + geocoding)
  - get_route                   (public tool: all paths, mocked externals)

Run with:
    pytest test/test_get_route.py -v
"""
...
class TestEncodePolyline:
    ...
    def test_precision_parameter(self):
        """Precision 5 vs 6 should yield different encodings for same points."""
...
class TestRouteStub:
    ...
    def test_road_factor_applied(self):
        """Stub multiplies straight-line by 1.3."""
...
class TestGeocodeAddress:
    ...
    def test_known_base_returns_from_cache(self):
        """Addresses in _BASE_COORDS_CACHE should be returned instantly."""
    ...
    def test_unknown_address_hits_nominatim(self):
        """Non-cached addresses delegate to Nominatim; mock it to avoid network.
        Nominatim is imported locally inside _geocode_address, so we patch
        the class in the geopy.geocoders namespace."""
...
class TestGetRoute:
    ...
    def test_coords_provided_uses_stub_or_real(self):
        """When both origin and destination coords are provided, must return a route."""
        result = self._parse(
    ...
    def test_unit_type_picks_nearest_base(self):
        """When unit_type is given, origin should be one of its bases."""
        result = self._parse(
    ...
    def test_destination_geocoding_failure_returns_error(self):
        """If destination cannot be geocoded, return an error dict."""
    ...
    def test_no_origin_uses_city_centre(self):
        """No origin info at all → falls back to city-centre coordinates."""
        result = self._parse(
            destination_address="",
            destination_lat=self.DEST_LAT, destination_lon=self.DEST_LON,
        )
    ...
    def test_origin_address_from_cache(self):
        """Providing a cached base name as origin_address should work."""
        result = self._parse(
            origin_address="Base Sur, Sevilla",
            destination_address="",
            destination_lat=self.DEST_LAT, destination_lon=self.DEST_LON,
        )
        expected = list(_BASE_COORDS_CACHE["Base Sur, Sevilla"])
