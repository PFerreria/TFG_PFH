"""
test_protocol_indexer.py
------------------------
Comprehensive unit tests for tools/protocol_indexer.py.

Covers:
  - _from_stub                 (always-available built-in table)
  - _load_cache                (file loading, validation, empty-path)
  - _from_cache                (exact match, severity fallback, miss)
  - query_protocol_index       (public tool: all three tiers, JSON output)

Run with:
    pytest test/test_protocol_indexer.py -v
"""
...
def isolate_cache(monkeypatch, tmp_path):
    """
    Redirect CACHE_PATH to a temp file and reset the module cache singleton
    before each test so tests don't bleed into each other.
    """
    _reset_module_cache()
...
class TestFromStub:
    ...
    def test_stub_is_a_copy_not_reference(self):
        """Mutating the returned dict should not affect the module _STUBS."""
        result = _from_stub("fire")
...
class TestLoadCache:
    ...
    def test_wrong_structure_returns_empty(self, tmp_path, monkeypatch):
        """A flat list instead of a nested dict should be treated as invalid."""
...
class TestFromCache:
    ...
    def _inject_cache(self, data: dict, monkeypatch):
        """Inject a fake cache dict directly into the module singleton."""
        pi._cache = data
    ...
    def test_severity_fallback(self, monkeypatch):
        """If 'fire/low' not in cache, should fall back to 'fire/critical'."""
        self._inject_cache({
            "fire": {
