"""
Unit tests for agents/analysis_agent.py.
"""
...
class TestLoadIncidentData:
    ...
    def test_loads_from_sqlite_when_db_exists(self, tmp_path):
        ...
        conn.execute(
            ...
            "(id TEXT, status TEXT, timestamp TEXT, incident_type TEXT, severity TEXT, data TEXT)"
        )
        rows = [
            (
                str(row["id"]), "processed", str(row["timestamp"]),
                row["incident_type"], row["severity"],
                json.dumps({
                    "latitude":          row["latitude"],
                    "longitude":         row["longitude"],
                    "address":           row["address"],
                    "response_time_min": row["response_time_minutes"],
