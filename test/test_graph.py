...
test_graph.py
-------------
Tests the IMERS LangGraph state machine using lightweight stub agents.
No LLM / HuggingFace token needed - all agent .run() methods are replaced
with deterministic stubs that return realistic mock outputs.

Tests covered:
  1. Happy path - transcript with clear location -> full pipeline completes
  2. Geo retry   - location initially unresolved -> retry succeeds
  3. Abort path  - no transcript and no audio -> abort_node triggered
  4. Severity reconciliation - procedure agent overrides NLP severity
  5. State shape  - final_report has all required keys
