...
analysis_runner.py
------------------
IMERS — Analysis Agent Lifecycle Manager
==========================================
This is the ONLY file that instantiates and runs the AnalysisAgent.
It has two jobs, both completely independent of the dispatch pipeline:

  JOB 1 — Dashboard refresh scheduler
  ------------------------------------
  Runs every N minutes (default: 60). Computes:
    - Hotspot map (grid clustering)
    - 7-day incident forecast
    - Response time KPIs
  Stores the result in memory (or Redis if configured).
  The FastAPI backend serves this cached result via GET /api/dashboard.

  JOB 2 — Post-dispatch incident annotation
  ------------------------------------------
  Triggered once per incident AFTER the pipeline has already returned its
  report to the operator. Computes:
    - Is this location a known hotspot?
    - How many similar incidents in the last 30 days?
    - What is the historical risk level for this area + type?
  Writes the result to the incident DB record asynchronously.
  The operator can view it on the dashboard minutes later.
...
--------------------
  ...
  Emergency call arrives
          ...
          ▼
  ┌----------------------------------┐
  │  IMERS Pipeline (LangGraph)      │   ← real-time, < 2 seconds
  │  TTS → NLP → Validate →         │
  │  [Procedure | Dispatch | Geo]    │
  │           └--► final_report      │
  └----------┬-----------------------┘
             │  post-dispatch hook fires
             │  (daemon thread, non-blocking)
             ▼
  ┌----------------------------------┐
  │  AnalysisAgent.annotate_async()  │   ← background, ~5-15 seconds
  ...
  ┌----------------------------------┐
  │  AnalysisAgent.generate_         │   ← scheduled, independent
  │      dashboard_data()            │
  │  stores in _dashboard_cache      │
  │  served by GET /api/dashboard    │
  └----------------------------------┘

How to wire this into the API server (api.py)
----------------------------------------------
  ...
  # At startup:
  runner   = AnalysisRunner(hf_token=HF_TOKEN)
  pipeline = IMERSPipeline(hf_token=HF_TOKEN)
  runner.attach_to_pipeline(pipeline)   # registers the post-dispatch hook
  runner.start_scheduler()              # begins the 60-min dashboard refresh
...
How to run standalone (without the pipeline, for testing or cron jobs)
----------------------------------------------------------------------

  # Generate fresh dashboard data once:
  python analysis_runner.py --dashboard

  # Annotate a specific incident by ID (looks up from DB):
...
class AnalysisRunner:
    """
    Lifecycle manager for the AnalysisAgent.

    Owns:
      - One AnalysisAgent instance
      - The dashboard refresh cache
      - The APScheduler job
      - The post-dispatch hook registration

    Does NOT own or reference IMERSPipeline internals.
    IMERSPipeline does not own or reference AnalysisRunner.
    The only coupling point is the post-dispatch hook, registered via
    pipeline.add_post_dispatch_hook() from the outside.
    """
    ...
    def refresh_dashboard(self) -> dict:
        """
        Compute fresh dashboard analytics and store in cache.
        Safe to call at any time — does not touch the pipeline.
    ...
    def get_dashboard_data(self) -> dict:
        """
        Return the latest cached dashboard data.
        Calls refresh_dashboard() on first call if cache is empty.
        """
    ...
    def start_scheduler(self, interval_minutes: int = 60) -> None:
        """
        Start a background APScheduler job that calls refresh_dashboard()
        every N minutes. Also runs once immediately on startup.
    ...
    def annotate_incident(
        ...
        """
        Enrich a completed incident with historical context.
        Runs in a background daemon thread — returns immediately.

        Args:
            incident_id:   e.g. "INC-00142"
            ...
            on_complete:   Optional callback receiving the annotation dict.
                           Use this to write to DB, push to WebSocket, etc.
        """
    ...
    def attach_to_pipeline(self, pipeline, on_complete=None) -> None:
        """
        Register the post-dispatch annotation hook on an IMERSPipeline instance.
        This is the ONLY coupling between the pipeline and the analysis runner,
        and it goes in one direction: the pipeline calls the runner after dispatch.
        The pipeline does not import or know about the AnalysisRunner class.

        Args:
            pipeline:    An IMERSPipeline instance.
            on_complete: Optional callback for each annotation result.
                         Defaults to a logger.info call if not provided.

        Example:
            runner.attach_to_pipeline(
                pipeline,
                on_complete=lambda a: db.execute(
                    "UPDATE incidents SET annotation=? WHERE id=?",
                    [json.dumps(a), a["incident_id"]]
                )
            )
        """
