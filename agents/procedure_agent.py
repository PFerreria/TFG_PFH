...
-------------------------
Procedure Agent — retrieves and interprets emergency protocols using RAG.

Uses ToolCallingAgent (single structured tool call per step) rather than
CodeAgent (which generates Python code and executes it — up to 16 LLM
round-trips per run). The fetch_protocol tool returns the full protocol text;
the LLM extracts steps and escalation flags in a single final_answer call.
Typical latency: 1-2 LLM calls vs up to 16 with CodeAgent.
...
class ProcedureAgent:
    """
    RAG-powered protocol retrieval and interpretation agent.

    Usage:
        from smolagents import InferenceClientModel
        model = InferenceClientModel("Qwen/Qwen2.5-72B-Instruct", token=HF_TOKEN)

        agent = ProcedureAgent(model)
        result = agent.run(
            incident_type="cardiac_arrest",
            severity="critical",
            transcript="Mi padre no respira...",
            extra_context="victim is elderly, third floor no elevator"
        )
    ...
    def _is_clean_step(s: object) -> bool:
        """Return True if *s* is a human-readable protocol step.

        Filters out two garbage sources that can appear in the ``steps`` list
        when the vector-search tier is used:
        1. Strings that begin with 'Context information is below' — these are
           raw ChromaDB retrieval context strings, not actionable steps.
        2. Strings containing more than 3 non-printable control characters
           (codepoints 0x00-0x1F, excluding \\t \\n \\r) — these indicate
           binary/OCR garbage extracted from PDFs.
        """
    ...
    def _fallback(self, incident_type: str, severity: str, error: str, transcript: str = "") -> dict:
        """Fetch protocol directly (bypassing the LLM) when the agent fails.

        Uses the actual cached protocol steps so the operator receives useful
        instructions even when the LLM provider returns an error.
        """
        try:
