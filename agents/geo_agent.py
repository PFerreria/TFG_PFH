...
agents/geo_agent.py
-------------------
Geographic Agent — resolves incident location and computes optimal routes
for every dispatched unit concurrently.

Why ToolCallingAgent?
  Location resolution and routing are pure tool-call tasks — no code
  generation required. The model reasons about ambiguous addresses (e.g.
  "the roundabout near the cathedral") and selects the correct geocoded
  result, then calls get_route for each unit.

Key feature — concurrent routing:
  Routes for multiple units are computed in parallel using ThreadPoolExecutor
  so total latency is max(single_route_time) not sum(all_routes_time).
  This is critical for the <2s pipeline target.
"""
...
class GeoAgent:
    """
    Location resolution and multi-unit routing agent.

    Usage:
        agent = GeoAgent(model)
        result = agent.run(
            transcript="Accidente en Calle Sierpes esquina Calle Tetuan, Sevilla",
            units=[
                {"id": "AMB-SVA-01", "base_location": "Hospital Virgen del Rocío"},
                {"id": "POL-01",     "base_location": "Comisaría Central"},
            ],
        )
        print(result["incident_address"])
        for r in result["routes"]:
            print(r["unit_id"], r["eta_minutes"], "min")
    """
    ...
    def run(
        ...
        Optimisation note:
            When ``known_lat`` and ``known_lon`` are provided the LLM call is
            skipped entirely — the agent only computes routes for the supplied
            coordinates.  This saves 1-3 s of LLM round-trip time per pipeline
            run when nlp_node already resolved the location (the common case).
        """
    ...
    def _compute_routes_parallel(
        ...
        """
        Compute routes for all units concurrently using a ThreadPoolExecutor.
        This keeps total latency at max(single_route) instead of sum(all_routes).
        """
        if not units:
            return []
