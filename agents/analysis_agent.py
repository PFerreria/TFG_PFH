...
------------------------
Data Analysis Agent — historical analytics for the operator dashboard.

ARCHITECTURAL DECISION
-----------------------
This agent is DELIBERATELY DECOUPLED from the real-time dispatch pipeline.

Why:
  Saving a life does not depend on whether a street is a statistical hotspot.
  Procedure, Dispatch, and Geo agents must complete in < 2 seconds.
  Historical trend analysis (pandas groupby, clustering, DB reads) takes
  5-15 seconds — unacceptable latency on the critical path.

Two independent use cases (never called from pipeline/graph.py):
  1. Background refresh — runs on a schedule (every hour via APScheduler)
     to update the dashboard: hotspot map, 7-day forecast, KPI charts.
     Entry point: agent.generate_dashboard_data()

  2. Post-dispatch annotation — called AFTER dispatch is confirmed, as a
     fire-and-forget daemon thread. Enriches the stored incident record
     with historical context without blocking the operator.
     Entry point: agent.annotate_incident_async(incident_id, incident_type, lat, lon)

Data source:
  Reads from incidents.db (SQLite) or incidents.csv fallback.
  Run with --generate to create a synthetic dataset for testing.

Dependencies:
    pip install smolagents pandas numpy scikit-learn apscheduler
"""
...
def _load_incident_data() -> pd.DataFrame:
    """Load incident data from DB (preferred) or CSV fallback.

    The incidents table stores only (id, status, timestamp, incident_type,
    severity, data) at the top level; all other fields (latitude, longitude,
    address, response_time) live inside the JSON 'data' blob.  We use
    json_extract() so callers always get a flat DataFrame with the expected
    column names.
    """
    if DB_PATH.exists():
        try:
...
def get_hotspot_analysis(grid_size_km: float = 0.3) -> str:
    """Detects geographical hotspots by clustering incident locations.
    Uses a grid-cell approach: cells with more incidents than average are flagged as hotspots.
...
def get_trend_forecast(forecast_days: int = 7) -> str:
    """Predicts daily incident counts for the next N days using historical patterns.
    Uses day-of-week seasonality and recent rolling averages.
...
def get_hourly_distribution(days_back: int = 30) -> str:
    """Computes the average incident count per hour of day for dashboard charting.
    ...
    Returns:
        JSON with keys:
          hourly (list[float]): 24 values, one per hour 0–23, each the average
            number of incidents that occurred at that hour across the analysis window.
...
class AnalysisAgent:
    ...
    Correct usage:
        ...
        # Scheduled dashboard refresh (every hour) ----------
        # Call from a cron job / APScheduler, store result in Redis/DB.
        # The frontend polls /api/dashboard, which returns the cached result.

        # Post-dispatch annotation (background thread) ------
        # Called AFTER the final_report is written and dispatch is confirmed.
        agent.annotate_incident_async(
            incident_id="INC-00142",
            incident_type="traffic_accident",
            lat=37.386, lon=-5.992,
            on_complete=lambda result: db.update_incident(result),
        )
    """
    ...
    def generate_dashboard_data(self) -> dict:
        """
        Generate a full analytics report for the operator dashboard.
        Run this on a schedule (every 30-60 minutes), cache in Redis, serve
        via GET /api/dashboard. Never call this from the dispatch pipeline.
    ...
    def annotate_incident_async(
        ...
        """
        Enriches a completed incident record with historical context in the
        background. Returns immediately — computation runs in a daemon thread.

        This is called AFTER dispatch is confirmed. The result is passed to
        on_complete (e.g. a DB write), never blocking the operator workflow.
    ...
    def _direct_enrich(
        ...
        """
        Compute historical context using tools directly (no LLM call).
        Fast enough for background annotation.
        """
        try:
...
def start_dashboard_scheduler(
    ...
    """
    Start a background scheduler that refreshes dashboard data every N minutes.
    Designed to be called once at API server startup.

    Args:
        agent:            An initialised AnalysisAgent instance.
        on_refresh:       Callback that receives the fresh dashboard dict.
                          Typically: lambda data: redis.set("dashboard", json.dumps(data))
        interval_minutes: How often to refresh (default 60).

    Example (in api.py):
        from agents.analysis_agent import start_dashboard_scheduler
        start_dashboard_scheduler(analysis_agent, lambda d: cache.set("dashboard", d))
    """
    try:
...
def generate_sample_data(n: int = 500, output_path: Path = CSV_PATH) -> pd.DataFrame:
    """Generate a synthetic incident dataset for testing.

    Temporal distribution is calibrated from 2019 Andalucía 112 statistics so that
    the generated data produces realistic hourly curves and seasonal forecasts.
    """
