...
def _extract_agent_json(raw) -> dict | None:
    """Strips markdown fences then parses the first JSON object; returns None if none found."""
...
def _extract_from_groq_error(exc: Exception) -> dict | None:
    """Salvages JSON from Groq 'tool_use_failed' errors where the model wrote its answer to 'failed_generation' instead of final_answer."""
    msg = str(exc)
...
def _extract_tool_call_from_groq_error(exc: Exception) -> tuple[str, dict] | None:
    """Parses (tool_name, args_dict) from Groq 400 non-standard <function=name> format; returns None if not found."""
    msg = str(exc)
