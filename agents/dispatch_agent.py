...
------------------------
Dispatch Agent — selects and validates emergency units based on incident
classification, victim count, and real-time unit availability.

Why ToolCallingAgent (not CodeAgent)?
  Dispatch is structured and deterministic: query availability, apply
  protocol rules, return an allocation. Tool calls are the right abstraction;
  the model validates the recommendation without executing Python code.
...
class DispatchAgent:
    """
    Unit allocation and dispatch recommendation agent.

    Usage:
        agent = DispatchAgent(model)
        result = agent.run(
            incident_type="traffic_accident",
            severity="critical",
            location="Avenida de la Constitución, Sevilla",
            victims=3,
        )
        print(result["units"])
        print(result["estimated_first_arrival"])
    """
    ...
    def _build_prompt(
        ...
        """Build the agent prompt from incident parameters."""
        return "\n".join([
            DISPATCH_SYSTEM_PROMPT,
            "",
            "=== INCIDENT ===",
