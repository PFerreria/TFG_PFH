"""Transcribes audio via Whisper cascade: Fireworks → Groq → HuggingFace → faster-whisper → openai-whisper."""
...
# Sevilla domain prompt + hotwords shared with the streaming transcriber
try:
    from realtime.streaming_transcriber import (
        WHISPER_PROMPT,
        WHISPER_HOTWORDS,
        _strip_hallucinations,
    )
except Exception:  # standalone use outside the package root
    WHISPER_PROMPT   = os.getenv("IMERS_WHISPER_PROMPT") or "Llamada de emergencia al 112 en Sevilla, España."
    WHISPER_HOTWORDS = os.getenv("IMERS_WHISPER_HOTWORDS") or ""
...
def _run_hf_whisper(audio_path: str, model_size: str = WHISPER_MODEL) -> TranscriptionResult:
    """Transcribes via HuggingFace ASR endpoint; language is detected server-side."""
...
def _run_faster_whisper(audio_path: str, model_size: str = WHISPER_MODEL) -> TranscriptionResult:
    ...
    if WHISPER_HOTWORDS:
        _kwargs["hotwords"] = WHISPER_HOTWORDS
    try:
        segments_iter, info = model.transcribe(audio_path, **_kwargs)
    except TypeError:  # older faster-whisper without hotwords support
        _kwargs.pop("hotwords", None)
        segments_iter, info = model.transcribe(audio_path, **_kwargs)
...
def _run_openai_whisper(audio_path: str, model_size: str = WHISPER_MODEL) -> TranscriptionResult:
    ...
    result = model.transcribe(audio_path, language=WHISPER_LANG,
                              initial_prompt=WHISPER_PROMPT, verbose=False)
    processing_ms = int((time.perf_counter() - t0) * 1000)
...
class TTSAgent:
    """Tries each Whisper backend in cascade order and returns the first successful transcription."""

    def __init__(self, model_size: str = WHISPER_MODEL):
        self.model_size = model_size

    def _resolve_audio_path(self, audio_path: str) -> str:
        """Resolves relative paths against DEFAULT_AUDIO_FOLDER; leaves absolute paths unchanged."""
    ...
    def _validate_audio_file(self, audio_path: str) -> Optional[str]:
        """Returns an error string if the file doesn't exist or has an unsupported format, else None."""
        path = Path(audio_path)
    ...
    def run(self, audio_path: str) -> TranscriptionResult:
        ...
        for fn, name in backends:
            try:
                ...
                result.transcript = _strip_hallucinations(result.transcript)
                logger.info(
