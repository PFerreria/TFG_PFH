...
class _VAD:
    """webrtcvad wrapper with an RMS-energy fallback."""
    FRAME_MS    = 30
    ...
    def is_speech(self, chunk_bytes: bytes) -> bool:
        """True if the chunk looks like speech."""
    ...
    def _vad_check(self, chunk_bytes: bytes) -> bool:
        """webrtcvad vote across 30 ms sub-frames."""
        speech_frames = 0
        total_frames  = 0
...
class CallReceiver:
    ...
    def start(self) -> None:
        """Begin receiving audio on a background thread."""
        self._running = True
        self._silent_chunks = 0
        target = {
            "mic":       self._run_mic,
    ...
    def stop(self) -> None:
        """Stop the receiver and join its thread."""
        self._running = False
    ...
    def _deliver(self, chunk: bytes) -> None:
        """Emit one chunk and update the silent-chunk counter."""
    ...
    def _run_mic(self) -> None:
        """Read PCM from the default microphone via PyAudio."""
        try:
            import pyaudio
        except ImportError:
    ...
    def _run_file(self) -> None:
        """Stream a WAV/MP3/OGG/FLAC file as chunks (real-time when self.realtime)."""
    ...
    def _run_socket(self) -> None:
        """Accept one TCP connection and read raw 16 kHz mono PCM from it."""
    ...
    def _run_websocket_stub(self) -> None:
        """No-op. WebSocket bytes are fed via push_bytes() from api.py."""
        logger.info(
    ...
    def push_bytes(self, data: bytes) -> None:
        """Feed raw PCM bytes from outside (WebSocket/HTTP). Buffers until a full chunk."""
    ...
    def flush(self) -> None:
        """Deliver any remaining buffered audio and fire on_end."""
        self._silent_chunks = 0
...
def _float32_to_int16(samples: np.ndarray, src_rate: int) -> np.ndarray:
    """Resample float32 mono samples to 16 kHz int16 PCM.

    Uses linear interpolation.  For higher-quality resampling (avoids aliasing
    artefacts when downsampling from 48 kHz) use scipy.signal.resample_poly instead.
    """
...
def _normalise_raw_pcm(data: bytes, src_rate: int) -> bytes:
    """Resample raw int16 PCM bytes to 16 kHz mono int16."""
    if src_rate == SAMPLE_RATE:
        return data
