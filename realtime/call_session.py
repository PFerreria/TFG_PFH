...
class CallSession:
    ...
    def stop(self) -> None:
        """End the session (flush remaining audio, fire final pipeline)."""
    ...
    def push_audio(self, audio_bytes: bytes) -> None:
        """Feed PCM bytes from an external source (WebSocket handler)."""
        if self._receiver:
            self._receiver.push_bytes(audio_bytes)

    def hangup(self) -> None:
        """Mark the call as ended by the caller."""
        if self._receiver:
            self._receiver.flush()

    @property
    def transcript(self) -> str:
        """The latest known transcript (live during the call, final after)."""
    ...
    def _on_partial(self, partial: str) -> None:
        """Receive a transcript update from StreamingTranscriber."""
        logger.debug(f"[CallSession {self.session_id}] Partial ({len(partial.split())} words)")

    def _on_pause(self) -> None:
        """Flush the transcriber when CallReceiver detects a speech pause."""
    ...
    def _on_early_trigger(self, transcript: str) -> None:
        """Run the preliminary pipeline on a background thread."""
    ...
    def _on_final_transcript(self, transcript: str) -> None:
        """Run the final pipeline on a background thread."""
    ...
    def _run_pipeline(self, transcript: str, report_type: str) -> None:
        """Run the pipeline, tag the result, and invoke on_report."""
...
class CallSessionManager:
    ...
    def create(self, source: str = "mic", **source_kwargs) -> CallSession:
        """Create, register, and return a new CallSession."""
        with self._lock:
            self._counter += 1
            session_id = (
    ...
    def close(self, session_id: str) -> None:
        """Stop and deregister the session with the given id."""
    ...
    def _handle_report(self, session_id: str, report_type: str, report: dict) -> None:
        """Forward a session report to the global on_report callback."""
        try:
