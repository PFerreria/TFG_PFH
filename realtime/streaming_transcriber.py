...
class StreamingTranscriber:
    ...
    def _transcribe_bytes(self, audio_bytes: bytes) -> str:
        """Run Whisper on the given PCM bytes using the provider cascade:
        Fireworks -> Local -> Groq -> HF.
        """
        if not audio_bytes:
            return ""
    ...
    def _deduplicate(self, existing: str, new_text: str) -> str:
        """Strip the leading words of new_text that already end existing."""
        ...
        for overlap_len in range(max_overlap, 0, -1):
            if existing_words[-overlap_len:] == [w.lower() for w in new_words[:overlap_len]]:
    ...
    def _maybe_fire_silence_trigger(self, reason: str) -> None:
        ...
        logger.info(
            ...
            f"{len(words)} words — sending transcript to pipeline"
        )
        try:
            self.on_early_trigger(current_transcript)
        except Exception as e:
