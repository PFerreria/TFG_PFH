...
dashboard/api.py
────────────────
IMERS — FastAPI Backend
========================
Run from the project root:
    python -m uvicorn dashboard.api:app --host 0.0.0.0 --port 8000 --reload

Set IMERS_MOCK_MODE=1 (default) to run without HF_TOKEN for UI development.
Set IMERS_MOCK_MODE=0 and provide GROQ_API_KEY / HF_TOKEN for the real pipeline.
"""
...
def _resolve_stale_active_incidents() -> None:
    ...
    Emergency response operations cannot realistically remain active longer than
    4 hours. Any incident still marked active/en_route beyond that threshold is
    resolved automatically. If this leaves zero active incidents in the DB,
    fresh active incidents are re-seeded so the demo always has live data.
    """
...
class _MockPipeline:
    ...
    def run_transcript(self, transcript: str, incident_id: str = None, is_preliminary: bool = False, **kwargs) -> dict:
        ...
        dispatch_data: dict = {}
        try:
            from tools.recommend_units import recommend_units as _ru
...
def _get_adapted_protocols() -> tuple[list[dict], str]:
    ...
    Tier logic mirrors the live agent's query_protocol_index priority:
      cache  — a structured JSON entry exists for this incident type
      vector — cache miss but ChromaDB has been ingested
      stub   — no cache, no vector; agent uses built-in stubs
    """
    cache: dict = {}
    try:
...
async def _queue_processor() -> None:
    """
    Background coroutine: waits for queue items and processes them one at a time.
...
def _wait_session_complete(session, timeout_s: float = 300.0) -> None:
    ...
    Falls through after *timeout_s* seconds so the queue processor never gets
    stuck permanently if a session crashes without transitioning to COMPLETE.
    """
...
async def upload_recording(request: Request, file: UploadFile = File(...)):
    ...
    The file is saved to data/recordings/uploads/ and kept after processing.
    """
...
async def ws_audio(ws: WebSocket):
    ...
    def _safe_push_report(sid, rt, r):
        """Push the pipeline report to the WebSocket, but only if the event loop
        is still running (guards against the shutdown race where the final
        pipeline run completes after Uvicorn has already closed the loop)."""
        if loop.is_closed():
            logger.warning(
