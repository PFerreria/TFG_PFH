 ...
 * js/history.js
 * -------------
 * Screen 2 — Historial de incidentes
 *
 * Responsibilities:
 *   - Merge HISTORY + active INCS into one sortable/filterable table
 *   - Apply text search, severity filter, and status filter
 *   - Sort any column ascending / descending on header click
 *   - Open the drill-down modal with timeline, agent note, confidence gauge
 *
 * Public API:
 *   renderHistory()             — re-render table with current filters/sort
 *   setHistFilter(type,val,btn) — update a filter and re-render
 *   sortHist(key)               — toggle sort on a column key
 *   openHistModal(id)           — open detail modal for a given incident ID
 *   closeHistModal()            — close the detail modal
 */
...
function renderHistory() {
  ...
  // Sort — fall back to ts/desc when no explicit sort is active
