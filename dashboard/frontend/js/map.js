 ...
 * Screen 1 — Live Map
 *
 * All data comes from globals populated by data.js (INCS, HOTSPOTS, KPI).
 * No values are hard-coded in this file.
 *
 * Public API:
 *   initMap()           — create Leaflet map once on DOMContentLoaded
 *   renderMapOverlays() — (re)draw hotspot circles + incident markers
 *   renderIncList()     — (re)render left sidebar incident list
 *   filterInc(sev,btn)  — change severity filter and re-render
 *   openDetail(id)      — open slide-in detail panel for one incident
 *   closeDetail()       — close detail panel
 *   renderSeverityBars()— populate right-panel severity breakdown
 *   renderTypeBars()    — populate right-panel type breakdown
 *   renderHsMini()      — populate right-panel hotspot mini-list
 */
...
function initMap() {
  ...
  leafMap = L.map("map-el", {
    ...
    maxBoundsViscosity: 0.85,   // rubberband resistance when panning outside
    minZoom: 11,                // prevent zooming out beyond city level
  ...
  // Update coordinates badge to actual map centre
  leafMap.on("move", () => {
...
function renderMapOverlays() {
  ...
  // Base markers — drawn from BASES global
  BASES.forEach(base => {
  ...
  // Hotspot halo circles — drawn from live HOTSPOTS global
  HOTSPOTS.forEach(hs => {
  ...
  // Incident markers — drawn from live INCS global (only within Sevilla)
...
function closeDetail() {
  ...
  selId = null;
  // Clear panel body so stale incident data is never visible if the panel
  // somehow becomes open again (race conditions, CSS, etc.)
...
/**
 * Severity breakdown bars — computed from live INCS global.
 * Replaces the previous hard-coded widths and counts in index.html.
 */
...
/**
 * Type breakdown bars — computed from live INCS global.
 */
  ...
  document.getElementById("type-bars").innerHTML = sorted.length
    ? sorted.map(([type, n], idx) => `
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:6px">
          <span style="font-family:var(--cond);font-size:8px;font-weight:500;color:var(--t2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${_escHtml(type)}</span>
          ...
          <span style="font-family:var(--mono);font-size:8px;color:var(--t3);width:14px;text-align:right">${n}</span>
        </div>`).join("")
    : `<div style="font-family:var(--mono);font-size:8px;color:var(--t3)">Sin datos</div>`;
}

/**
 * Hotspot mini-list — reads from live HOTSPOTS global.
 */
...
async function confirmArrival(id) {
  // Close the panel immediately — do not wait for the network round-trip.
  // This prevents the panel from staying open (and then showing a different
  // incident) when a WebSocket broadcast has already removed the incident
  // from INCS before the fetch resolves (INCS.find returns null → the old
  // code's `if (inc)` guard would skip closeDetail entirely).
  closeDetail();
  ...
  try {
    ...
    // Incident may have already been removed from INCS by a WS broadcast;
    // update whichever copy we can find and always refresh the UI.
    const inc = INCS.find(i => i.id === id);
    if (inc) {
      inc.status = "resolved";
      upsertIncident(inc);
