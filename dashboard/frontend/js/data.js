 ...
 * js/data.js
 * ----------
 * Data layer. Fetches all application data from the FastAPI backend on load.
 *
 * Globals populated by this file (read by map.js, history.js,
 * predictions.js, knowledge.js):
 *
 *   INCS      — active + recent incidents      ← GET /api/incidents/active
 *   HISTORY   — historical incidents           ← GET /api/incidents/history
 *   HOTSPOTS  — hotspot cluster analysis       ← GET /api/dashboard (.hotspots)
 *   FORECAST  — 7-day incident forecast        ← GET /api/dashboard (.forecast)
 *   HOURLY    — 24h distribution array         ← GET /api/dashboard (.hourly)
 *   KB        — protocol / knowledge base      ← GET /api/protocols
 *   KPI       — KPI snapshot object            ← GET /api/dashboard (.kpis)
 *
 * Load sequence
 * -------------
 * app.js calls loadAllData() on DOMContentLoaded. It resolves all fetches in
 * parallel, then calls initScreens() which bootstraps every module that reads
 * these globals. Nothing renders before the data is ready.
 *
 * Error handling
 * --------------
 * If any endpoint is unreachable, the corresponding global is set to [] / {}
 * and a console.error is emitted. The dashboard renders with whatever data
 * did arrive rather than crashing completely.
 */
...
let INCS     = [];   // active incidents
let HISTORY  = [];   // historical incidents
let HOTSPOTS = [];   // hotspot clusters with risk scores
let FORECAST = [];   // [{date, day_of_week, predicted_incidents}, ...]
let HOURLY   = [];   // 24 numbers, one per hour of day (average incident count)
let KB       = [];   // protocol documents from the RAG index
let KB_ADAPTED = []; // adapted protocols from protocol_indexer/cache
let KB_RAW     = []; // raw source documents in protocol_index folder
let KB_COVERAGE = {}; // knowledge-base health: doc/chunk counts, per-type tier
let KPI      = {};   // {overall: {mean, median, p90, count}, trend, by_severity}
let BASES    = [];   // unit base locations
let DASHBOARD_META = {}; // {historical_daily_avg, generated_at, source}
...
/**
 * Fetch all data sources in parallel. Called once by app.js on startup.
 * Returns when all requests have settled (success or failure).
 */
async function loadAllData() {
  ...
  // Active incidents — exclude any that have no real address
  ...
  // Dashboard snapshot (hotspots + forecast + hourly + kpis)
...
// -- Normalisation — map API field names to the names the UI expects -----------
//
// The API uses snake_case and explicit field names from the pipeline output.
// The UI was built with shorter camelCase-ish fields. These functions bridge
// the two without changing either side.

// Spanish display names for incident types (keys are API snake_case values)
const _INC_TYPE_ES = {
...
function _normaliseIncident(raw) {
  ...
  // Format units as: "BOM (5 min) - SVB (4 min) - ZETA (3 min)"
  // Label precedence: u.subtype (SVA/SVB/VIR/ZETA/MOTO/UME/BUL/BUP/AEA/FSV/UMES)
  //   → high-level type fallback → raw type string.
  const _TYPE_LABEL = {
    ambulance_sva: "SVA",
    ambulance_svb: "SVB",
    police:        "ZETA",
  ...
  // Fix 6: use protocol text/key_actions/escalation instead of the internal
  // source identifier (which was "rag" or "stub" — not operator-readable).
  ...
  if (typeof raw.protocol === "string") {
    ...
    // key_actions may be plain strings (from output_node) or {step,action,priority}
    // dicts if the pipeline version pre-dates the normalisation fix.
  ...
  // Fix 5: build Google Maps directions URL from incident coordinates/address.
...
function _normaliseHourly(raw) {
  // API returns either an array of 24 numbers or an array of {hour, count} objects
...
// -- Refresh helpers (called by app.js WebSocket handler) ----------------------

/**
 * Prepend a new incident report (from WebSocket) to INCS.
 * Removes any existing entry with the same ID to avoid duplicates
 * (a preliminary report may be superseded by a final report).
 */
// Placeholder values that indicate a genuinely missing address
...
function upsertIncident(report) {
  ...
  if (norm.status === "resolved" || norm.status === "complete" || norm.status === "resuelto") {
    ...
    // Do not add incidents with no real address to the live list —
    // they are pending operator input in the dispatch modal.
    if (!_hasRealAddress(norm)) {
      // Keep existing entry if one already had an address (don't downgrade)
      const existing = INCS.find(i => i.id === norm.id);
      if (existing && _hasRealAddress(existing)) return;
      // Otherwise just remove any stale copy without adding the no-addr version
      INCS = INCS.filter(i => i.id !== norm.id);
      return;
    }
...
/**
 * Refresh just the active incidents list from the server.
 * Called by app.js every 60s as a polling fallback if the WebSocket drops.
 */
...
/**
 * Refresh dashboard analytics (hotspots, forecast, KPIs).
 * Called by app.js when the Predictions tab is opened for the first time
