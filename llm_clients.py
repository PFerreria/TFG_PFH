"""Provider-cascade model: tries Fireworks → Ollama → Groq → HuggingFace on each generate() call."""
...
def _ollama_available_models() -> set[str]:
    """Returns locally pulled Ollama model tags, cached for _OLLAMA_CACHE_TTL seconds."""
...
def _pick_ollama_model(groq_model_id: str) -> str | None:
    """Maps a Groq model ID to the best available Ollama tag (exact or quantized prefix match)."""
...
def _build_groq_fallback_chain(model_id: str) -> list[str]:
    """Returns degradation chain for a Groq model ID, falling back to smaller models on rate-limit."""
    chain = [model_id]
    degradation = {
...
class GroqModel(OpenAIServerModel):
    """Groq-only model with exponential-backoff retry and HuggingFace failover on exhaustion."""
